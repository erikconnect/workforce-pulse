                ...
                {citySignals.map((signal) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">{signal.label}</p>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
            <div className="glass-panel card-hover-lift rounded-[28px] border border-white/35 p-4 dark:border-white/10">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-2xl bg-white/30 p-3 dark:bg-white/5">
                  <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Critical roles</p>
                  <p className="mt-1 text-xl font-semibold text-red-600 dark:text-red-300">{liveCriticalRoles}</p>
                </div>
                <div className="rounded-2xl bg-white/30 p-3 dark:bg-white/5">
                  <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Training gaps</p>
                  <p className="mt-1 text-xl font-semibold text-amber-700 dark:text-amber-300">{liveTrainingGaps}</p>
                </div>
                <div className="rounded-2xl bg-white/30 p-3 dark:bg-white/5">
                  <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Critical sectors</p>
                  <p className="mt-1 text-xl font-semibold">{liveCriticalSectors.length}</p>
                </div>
                <div className="rounded-2xl bg-white/30 p-3 dark:bg-white/5">
                  <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Workers mapped</p>
            ...
            <div className="glass-panel card-hover-lift rounded-[28px] border border-white/35 p-4 dark:border-white/10">
              ...
              <div className="space-y-3">
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Top departments</p>
                  <div className="flex flex-wrap gap-1.5">
                    {topDepartments.length > 0 ? topDepartments.map((dept) => (
                      <Badge
                        key={dept.department}
                        variant="outline"
                        className="border-white/35 bg-white/40 text-xs dark:border-white/10 dark:bg-white/5"
                      >
                ...
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Rising skills</p>
                  <div className="flex flex-wrap gap-1.5">
                ...
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Live pressure signals</p>
                  <div className="flex flex-wrap gap-1.5">
                    <Badge variant="outline" className="border-white/35 bg-white/40 text-xs dark:border-white/10 dark:bg-white/5">
                ...
                <CardContent className="p-3 space-y-3">
                  ...
                  <div className="border-t border-border/70 pt-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Active layers</p>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {layers.filter((layer) => layer.enabled).map((layer) => (
                        <span
                          key={layer.id}
                          className="rounded-full border border-white/35 bg-white/45 px-2 py-1 text-xs font-medium text-foreground/80 dark:border-white/10 dark:bg-white/5"
                        >
                          {layer.label}
                        </span>
                      ))}
                  ...
                  <div className="border-t border-border/70 pt-2 space-y-1.5">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Sector Breakdown</p>
                    {liveCriticalSectors.map((s) => (
                      <div key={s.id} className="flex items-center justify-between rounded-xl bg-red-50/70 px-2 py-1.5 text-xs dark:bg-red-950/20">
                        ...
                        <Badge variant="outline" className="text-xs text-red-600 border-red-300 dark:text-red-400 dark:border-red-700 ml-1">
                          {s.cityOpenJobs} open
                        </Badge>
                      </div>
                    ))}
                    {liveWatchSectors.map((s) => (
                      <div key={s.id} className="flex items-center justify-between rounded-xl bg-amber-50/70 px-2 py-1.5 text-xs dark:bg-amber-950/20">
                        <span className="truncate">{s.name}</span>
                        <Badge variant="outline" className="text-xs text-amber-600 border-amber-300 dark:text-amber-400 dark:border-amber-700 ml-1">
                          {s.cityOpenJobs} open
                        </Badge>
                      </div>
                    ))}
        ...
        {isAdmin && (
          <div className="pointer-events-none absolute bottom-3 left-3 right-3 z-[1000] hidden xl:flex items-end justify-between gap-3">
            <div className="pointer-events-auto glass-panel flex max-w-[540px] flex-wrap items-center gap-2 rounded-2xl border border-white/35 bg-white/75 px-3 py-2 shadow-[0_18px_34px_rgba(0,0,0,0.12)] backdrop-blur-xl dark:border-white/10 dark:bg-black/30">
              ...
              <span className="rounded-full bg-white/70 px-2 py-1 text-xs text-muted-foreground dark:bg-white/5">
            ...
            <div className="pointer-events-auto glass-panel flex items-center gap-2 rounded-2xl border border-white/35 bg-white/75 px-3 py-2 text-xs text-muted-foreground shadow-[0_18px_34px_rgba(0,0,0,0.12)] backdrop-blur-xl dark:border-white/10 dark:bg-black/30">
